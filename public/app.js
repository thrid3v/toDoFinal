...
async function addPersonalTodo() {
    ...
    const title = document.getElementById('personalTodoTitle').value;
    const description = document.getElementById('personalTodoDescription').value;
    ...
    try {
        ...
        if (response.ok) {
            document.getElementById('personalTodoTitle').value = '';
            document.getElementById('personalTodoDescription').value = '';
            loadTodos();
        } else {
            alert('Failed to create personal todo');
        }
    } catch (error) {
        console.error('Error creating personal todo:', error);
        alert('Error creating personal todo');
    }
}
...
async function loadTodos() {
    ...
    try {
        ...
        const assignedTodos = todos.filter(todo => todo.assignedTo.id === currentUser.id && !todo.isPersonal);
        const personalTodos = todos.filter(todo => todo.isPersonal);

        displayTodos('assignedTodosList', assignedTodos);
        displayTodos('personalTodosList', personalTodos, true);
    } catch (error) {
        console.error('Error:', error);
    }
}
...
function displayTodos(containerId, todos, isPersonal = false) {
    const container = document.getElementById(containerId);
    container.innerHTML = '';

    todos.forEach(todo => {
        const todoElement = document.createElement('div');
        todoElement.className = `todo-item ${todo.completed ? 'completed' : ''} ${isPersonal ? 'personal' : ''}`;
